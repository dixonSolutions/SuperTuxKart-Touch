                             ...
                             const core::vector2df &scaling)
{
    ...
    for (unsigned int i = 0; i < m_device->getButtonsCount(); i++)
    {
        ...
        if (button->type == MultitouchButtonType::BUTTON_STEERING)
        {
            if (m_use_glass_ui && m_glass_stick_base_tex && m_glass_stick_knob_tex)
            {
                // Glass virtual stick: base + knob offset by axis (left/right/forward).
                core::rect<s32> base_coords(pos_zero, m_glass_stick_base_tex->getSize());
                video::SColor base_color(255, 255, 255, 255);
                draw2DImage(m_glass_stick_base_tex, btn_pos, base_coords, NULL,
                            base_color, true);

                const float knob_scale = 0.42f;
                const int knob_w = (int)(button->width * knob_scale);
                const int knob_h = (int)(button->height * knob_scale);
                const float travel = 0.28f;
                const float cx = button->x + button->width * 0.5f
                    + button->axis_x * button->width * travel;
                const float cy = button->y + button->height * 0.5f
                    + button->axis_y * button->height * travel;
                core::rect<s32> knob_pos((int)(cx - knob_w * 0.5f),
                                         (int)(cy - knob_h * 0.5f),
                                         (int)(cx + knob_w * 0.5f),
                                         (int)(cy + knob_h * 0.5f));
                core::rect<s32> knob_coords(pos_zero, m_glass_stick_knob_tex->getSize());
                video::SColor knob_color(255, 255, 255, 255);
                if (button->pressed)
                    knob_color = video::SColor(255, 180, 230, 255);
        ...
        else
        {
            ...
            if (btn_texture)
            {
                video::ITexture* btn_bg = NULL;
                if (m_use_glass_ui)
                {
                    if (can_be_pressed && button->pressed && m_glass_btn_focus_tex)
                        btn_bg = m_glass_btn_focus_tex;
                    else if (button->type == MultitouchButtonType::BUTTON_FIRE &&
                             m_glass_btn_item_tex)
                        btn_bg = m_glass_btn_item_tex;
                    else if (button->type == MultitouchButtonType::BUTTON_SKIDDING &&
                             m_glass_btn_drift_tex)
                        btn_bg = m_glass_btn_drift_tex;
                    else if (button->type == MultitouchButtonType::BUTTON_NITRO &&
                             m_glass_btn_nitro_tex)
                        btn_bg = m_glass_btn_nitro_tex;
                    else if (button->type == MultitouchButtonType::BUTTON_LOOK_BACKWARDS &&
                             m_glass_btn_look_tex)
                        btn_bg = m_glass_btn_look_tex;
                    else
                        btn_bg = m_glass_btn_tex ? m_glass_btn_tex : m_bg_button_tex;
                }
                else
                {
                    btn_bg = (can_be_pressed && button->pressed) ?
                ...
                // Slightly inset icon over glass plate for a cleaner look.
                core::rect<s32> icon_pos = btn_pos;
                if (m_use_glass_ui)
                {
                    const int inset_x = button->width / 8;
                    const int inset_y = button->height / 8;
                    icon_pos = core::rect<s32>(button->x + inset_x,
                                               button->y + inset_y,
