...
#
# STKUpdateChecker reads GitHub's "latest release" feed, parses the tag as a dotted
...
REPO="${UPDATE_FEED_REPO:-dixonSolutions/SuperTuxKart-Touch}"
...
if not re.fullmatch(r"\d+(\.\d+)*", version):
    problems.append(
        f'tag {tag!r} does not parse as a dotted version; STKUpdateChecker would '
