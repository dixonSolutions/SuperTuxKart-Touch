...
FLATPAK_REMOTE_URL="${FLATPAK_REMOTE_URL:-https://dixonSolutions.github.io/SuperTuxKart-Touch/flatpak}"
FLATPAK_APP_ID="${FLATPAK_APP_ID:-io.github.dixonSolutions.SuperTuxKartTouch}"
FLATPAK_REMOTE_NAME="${FLATPAK_REMOTE_NAME:-supertuxtouch}"
...
test -n "$X86_BUNDLE" && test -f "$X86_BUNDLE" || { echo "Missing x86_64 bundle (--x86)" >&2; exit 1; }
test -n "$A64_BUNDLE" && test -f "$A64_BUNDLE" || { echo "Missing aarch64 bundle (--aarch64)" >&2; exit 1; }
...
<!DOCTYPE html>
<html lang="en">
<head><meta charset="utf-8"><title>SuperTuxTouch Flatpak remote</title></head>
<body>
<h1>SuperTuxTouch Flatpak remote</h1>
<p>${VERSION_NOTE}
App id <code>${FLATPAK_APP_ID}</code> — separate from Flathub <code>net.supertuxkart.SuperTuxKart</code>.
